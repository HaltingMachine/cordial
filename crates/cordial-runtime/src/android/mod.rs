...
pub fn backend() -> Backend {
    ...
    *CHOSEN.get_or_init(|| {
        let b = if std::env::var_os("WAYLAND_DISPLAY").is_some() {
            Backend::Wayland
        } else {
            Backend::X11
        };
        match b {
            Backend::Wayland => println!("[android] display backend: Wayland (WAYLAND_DISPLAY is set)"),
            Backend::X11 => println!(
                "[android] display backend: X11 (no WAYLAND_DISPLAY; see ADR-011 — this is the diagnostic fallback, not a supported target)"
            ),
        }
        b
    })
