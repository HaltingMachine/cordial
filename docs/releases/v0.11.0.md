...
**Nothing in 0.11.0 is signed.** Keyless signing of every release download
landed one commit after this tag was cut, so it arrives in the next release, not
this one -- every package here is unsigned and the README explains what that
does and does not protect you from. The Flatpak remote and the APT repository
stay unsigned beyond that too: they need an OpenPGP key, which Sigstore cannot
supply and which does not exist yet.
