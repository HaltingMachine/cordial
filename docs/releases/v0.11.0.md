...
**The Flatpak remote and the APT repository are not signed.** The downloads on
this release page are -- every package has a `.cosign.bundle` beside it and the
README says how to check one -- but the two repositories you can add to your
system need an OpenPGP key, which does not exist yet. The README explains what
